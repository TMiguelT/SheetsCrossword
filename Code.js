...
function insertClueSquares(ss, entries){
  ...
  for (let entry of entries){
    ...
    if (entry.direction == 'across'){
      ...
      entryRange = ss.getRange(entry.position.y + 1, entry.position.x + 1, 1, entry.length,);
      acrossClues.push(entry);
    }
    else {
      firstCell = ss.getRange(entry.position.y + 1, entry.position.x + 1, 1, 1);
      entryRange = ss.getRange(entry.position.y + 1, entry.position.x + 1, entry.length, 1);
...
function insertClueText(ss, clues, startRow, startColumn, title){
  ...
  ss.getRange(clueY++, clueX).setValue(title).setFontWeight("bold");
...
function addToSheet(crossData){  
  ...
  const crossRange = ss.getRange(1, 1, crossData.dimensions.rows, crossData.dimensions.cols);
...
function showPrompt() {
  ...
  var result = ui.prompt('Input the URL of the crossword you want to convert:', ui.ButtonSet.OK_CANCEL);
